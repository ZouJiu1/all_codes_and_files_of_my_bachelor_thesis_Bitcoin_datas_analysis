...
class SampleSpider(scrapy.Spider):
    ...
    def start_requests(self):
        df = pd.read_csv(r'E:\Block_D_value_Time.csv')
        a = list(set(np.arange(0, 507999, 1)) - set(df['Height'].values))
        x = 'file:///D:/D/data/'
        df = pd.DataFrame(columns=['Height', 'Input', 'Output', 'Sum', 'Time'])
        df.to_csv(r'e:\364001-367000.csv', index=False)
        urls = []
        for i in a:
    ...
    def parse(self, response):
        ...
        for i in response.xpath('//div/div/div[contains(@id,"tx-")]'):
            ...
            if 'Unable to decode input address'  in text:
                continue
        ...
        print(height)
        f.to_csv(r'e:\364001-367000.csv', index=False,header=None, mode='a')#, mode='a'